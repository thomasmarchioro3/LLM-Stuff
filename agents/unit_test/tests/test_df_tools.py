...
class TestDataFrameTools(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.runtime = RunTimeState(df=df)
    ...
    def test_pass(self):
        ...
        input_state = ChatState(messages=[HumanMessage(content="'Sup?")])
        ...
        logging.debug(
            f"{self.__class__.__name__} (test_pass): Last message - {last_message}"
        )
    ...
    def test_runtime_change(self):
        ...
        input_state = ChatState(
            messages=[HumanMessage(content="What is the dataframe length?")]
        )
        ...
        last_message = messages[-1]
        logging.debug(
            f"{self.__class__.__name__} (test_runtime_change): Last message - {last_message}"
        )
