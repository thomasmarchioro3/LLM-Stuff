...
def build_tools(runtime: RunTimeState):
    ...
    @tool
    def get_df_property(
        prop: Literal["len", "unique", "value_counts"], column: Optional[str] = None
    ):
        ...
        match prop:
            ...
            case "unique":
                if column is None:
                    return (
                        f"ERROR: column must be specified to get property {prop}."
                        f"column must be specified to get property {prop}."
                    )
